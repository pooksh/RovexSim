...
public class TaskGeneratorCustomInspector : Editor
{
    ...
    public override void OnInspectorGUI() {
        ...
        if (GUILayout.Button("Generate Tasklist")) {
            ...
            Transform[] waypoints = new Transform[objs.Length];
            ...
            for (int i = 0; i < numTasks; i++) {
                ...
                int originIndex = UnityEngine.Random.Range(0,objs.Length);
                int destinationIndex = UnityEngine.Random.Range(0,objs.Length);
                Transform origin = objs[originIndex].transform;
                Transform destination = objs[destinationIndex].transform;
                while (origin.position.x == destination.position.x && origin.position.y == destination.position.y) {
                    destinationIndex = UnityEngine.Random.Range(0,objs.Length);
                    destination = objs[destinationIndex].transform;
                }
